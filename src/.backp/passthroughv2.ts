...
// TODO: retirer si ça marche pas
export type CompatibleZodTypeWithoutPassthrough =
    | Extract<ZodTypeCategory3, ZodArray<SomeType> | ZodSet<SomeType>>
    | ZodTypeCategory0
    | ZodTypeCategory1
    | ZodTypeCategory2_5
    | ZodTypeCategoryCasseCouille

// TODO: en fait le probleme etait pas la mais peut être utile...
type Test2<TZodType extends SomeType, TCastZodType extends SomeType> =
    GetZodTypeValue<TZodType> extends GetZodTypeValue<TCastZodType> ? TZodType : never
...
export const ZodPassthrough = {
    ...
    pass: <TSchema extends CompatibleZodType>(
        schema: TSchema
    ): Test2<TSchema, CompatibleZodTypeWithoutPassthrough> => {
        if (!ZodPassthrough.is(schema)) {
            return schema as Test2<TSchema, CompatibleZodTypeWithoutPassthrough>
        }
        ...
        while (true) {
            ...
            if (ZodPassthrough.is(inner)) {
                ...
                return inner as Test2<TSchema, CompatibleZodTypeWithoutPassthrough>
            }
        }
    },
} as const
