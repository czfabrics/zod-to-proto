...
type GetFirstInUnion<T> = TuplifyUnion<T>[0]

type ContinueRecursiveRecordDebug<TChildrenRecord extends Test> = GetFirstInUnion<
    {
        [TKey in keyof TChildrenRecord]: TChildrenRecord[TKey] extends CheckZodSchema<
            TChildrenRecord[TKey]
        >
            ? never
            : CheckZodSchema<TChildrenRecord[TKey]>
...
type ContinueRecursiveArrayDebug<TChildrenArray extends Test2> = GetFirstInUnion<
    {
        [TKey in keyof TChildrenArray]: TChildrenArray[TKey] extends CheckZodSchema<
            TChildrenArray[TKey]
        >
            ? never
            : CheckZodSchema<TChildrenArray[TKey]>
    }[number]
>
...
type ContinueRecursiveSingleDebug<TChild extends $ZodType> =
    TChild extends CheckZodSchema<TChild> ? never : CheckZodSchema<TChild>
...
export type CheckZodSchema<TZodType extends SomeType> =
    TZodType extends NotNestedAnyZodCliItem
        ...
        : TZodType extends PassthroughZodTypeCategory1<
          ...
          ? TChildrenRecord extends ContinueRecursiveRecord<TChildrenRecord>
              ? TZodType
              : ContinueRecursiveRecordDebug<TChildrenRecord> // TODO
          : TZodType extends PassthroughZodTypeCategory2<
            ...
            ? TChild1 extends ContinueRecursiveSingle<TChild1>
                ? TChild2 extends ContinueRecursiveSingle<TChild2>
                    ? TZodType
                    : ContinueRecursiveSingleDebug<TChild2> // TODO
                : ContinueRecursiveSingleDebug<TChild1> // TODO
            : TZodType extends PassthroughZodTypeCategory2_5<
              ...
              ? TChildrenArray extends ContinueRecursiveArray<TChildrenArray>
                  ? TZodType
                  : ContinueRecursiveArrayDebug<TChildrenArray> // TODO
              : TZodType extends PassthroughZodTypeCategory3<
                ...
                ? TChild extends ContinueRecursiveSingle<TChild>
                    ? TZodType
                    : ContinueRecursiveSingleDebug<TChild> // TODO
                : TypeDebuggingError<`6- This Zod type '${GetZodTypeValue<TZodType>}' is not supported`>
